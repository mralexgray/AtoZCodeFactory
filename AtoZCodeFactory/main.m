...
- (allRootsBlock) rootEnumeratorBlock {

	return ^(AZNode *cat) { 
		[self.expansions enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
			AZNode *cat = AZNode.new; cat.value
	self.expansions 
}
...
-  (NSDictionary*) expansions 						{	NSString *e = nil;	NSPropertyListFormat fmt;
	return _expansions = [NSPropertyListSerialization propertyListFromData:[NSData dataWithContentsOfFile:_plistPath] mutabilityOption:NSPropertyListMutableContainersAndLeaves format:&fmt errorDescription:&e];
} 															/* parse the plist */
-        (AZNode*) root 								{ return _root = AZNode.new; _root.key = nil; 
	__block AZNode *cat, *def;  

	_allCatsBlock = ^(AZNode* cat) {
		[self.expansions enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
			cat = AZNode.new; 
			}:^(id key, id objs, BOOL *s){		
	};
	_allCatsBlock(
	_allKeywords = _allKeywords ?: NSMutableArray.new; _allReplacements = _allReplacements ?: NSMutableArray.new;
	
	return  [self.expansions enumerateKeysAndObjectsUsingBlock:^(id key, id objs, BOOL *s){		cat = AZNode.new; [_allKeywords addObject:cat.key = key];
		[objs enumerateKeysAndObjectsUsingBlock:^(NSString *macro,NSString *expansion, BOOL*s){ 
			[cat.children addObject:def = AZNode.new]; def.key = macro; def.value = expansion;	
		}];
...
- (NSDictionary*) filteredRoot {
	/*
	NSString*w; _matched = 0;  
 	w = [w = [_searchField stringValue] isEqualToString:@""] ? nil : w;									/*builds root, and matches searches simultaneously * /
